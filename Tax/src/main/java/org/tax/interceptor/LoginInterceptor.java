		...
		if (cookies != null && cookies.length > 0) {
			for (Cookie cookie : cookies) {
				if (cookie.getName().equals(CookieConst.USER)) {
					...
					String userId = URLDecoder.decode(cookie.getValue(),
							"UTF-8").split(";")[0];
					LOGGER.debug("userId :" + userId);
					MySession session = SessionControl.getInstance()
							.getSession(userId);
					if (session == null) {
						session = new MySession(UUIDUtil.genUUID());
						TaxUserKey key = new TaxUserKey();
						key.setId(userId);
						session.setAttribute(SessionConst.USER,
							mapperFactory.getTaxUserMapper().selectByPrimaryKey(key));
